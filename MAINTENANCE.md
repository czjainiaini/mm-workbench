...
| R-008 | P2 | 部分缓解 | 已加入零依赖离线冒烟测试与 GitHub Actions，覆盖语法、标记唯一性、静态资源 MIME、路径隔离和 LLM 中继安全边界；`workbench.html` 仍是大单文件，后续应在测试保护下分模块。 |
...
- 首次体验改为可操作的四步引导：离线示例、即时换色、撤销练习、完成反馈；不再要求新手先登录真站。
- 沙盒空状态提供离线、聊天页、创作页三条明确入口，并记住上次使用的模式。
...
4. P2：在现有离线冒烟测试与 GitHub Actions 上继续补包校验、角色 ID 和发布模拟；覆盖稳定后再拆分 `workbench.html`。
